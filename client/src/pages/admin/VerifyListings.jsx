...
const VerifyListings = () => {
  ...
  const fetchListings = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5001/api/v1/admin/pending-listings', {
  ...
  const handleVerify = async (listingId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:5001/api/v1/admin/listings/${listingId}/verify`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleReject = async (listingId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:5001/api/v1/admin/listings/${listingId}/reject`, {
        headers: { Authorization: `Bearer ${token}` }
      });
